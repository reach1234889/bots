...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('Nebula_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('NebulaBot')
...
WATERMARK = "Nebula VPS Service"
WELCOME_MESSAGE = "Welcome To Nebula! Get Started With Us!"
...
class Database:
    ...
    def _create_tables(self):
        ...
        self.conn.commit()
        # Ensure ssh_port column exists
        try:
            self.cursor.execute("ALTER TABLE vps_instances ADD COLUMN ssh_port TEXT")
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"Nebula/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with Nebula customization"""
    try:
        ...
        # Set Nebula customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up Nebula customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up Nebula customization...")
        ...
        hostname_cmd = f"echo 'Nebula-{vps_id}' > /etc/hostname && hostname Nebula-{vps_id}"
...
bot = NebulaBot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Nebula VPS"))
...
@app_commands.describe(
    ...
    use_custom_image="Use custom Nebula image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'Nebula-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
        ...
        await status_msg.edit(content="🔧 Container created. Setting up Nebula environment...")
        await asyncio.sleep(5)
        ...
        try:
            ...
            embed.add_field(name="🔌 Direct SSH", value=f"```ssh {username}@{SERVER_IP} -p {ssh_port}```", inline=False)
            embed.add_field(name="ℹ️ Note", value="This is a Nebula VPS instance. You can install and configure additional packages as needed.", inline=False)
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your Nebula VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All Nebula VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
4. You will be connected to your Nebula VPS

Or use direct SSH:
```ssh {username}@{SERVER_IP} -p {vps.get('ssh_port', '22')}```".format(username=vps['username'], SERVER_IP=SERVER_IP)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Nebula System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your Nebula VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="Nebula Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling Nebula bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "Nebula-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"Nebula VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.followup.send("✅ Nebula VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ Nebula VPS stopped successfully!", ephemeral=True)
        except Exception as e:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"Nebula VPS Restarted - {self.vps_id}", color=discord.Color.blue())
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling Nebula VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'Nebula-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"Nebula VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling Nebula VPS {self.vps_id}: {str(e)}")
                except:
