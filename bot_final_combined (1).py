...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('AMD_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('AMDBot')
...
WATERMARK = "AMD VPS Service"
WELCOME_MESSAGE = "Welcome To AMD! Get Started With Us!"
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'AMD.db'
BACKUP_FILE = 'AMD_backup.pkl'
...
async def build_custom_image(vps_id, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"AMD/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, vps_id=None, use_custom_image=False):
    """Enhanced container setup with AMD customization"""
    try:
        ...
        # Set AMD customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up AMD customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up AMD customization...")
        ...
        hostname_cmd = f"echo 'AMD-{vps_id}' > /etc/hostname && hostname AMD-{vps_id}"
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ AMD VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ AMD VPS setup completed successfully!")
...
bot = AMDBot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="AMD VPS"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 AMD VPS Bot Commands", color=discord.Color.blue())
...
@app_commands.describe(
    ...
    use_custom_image="Use custom AMD image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating AMD VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'AMD-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
        ...
        await status_msg.edit(content="🔧 Container created. Setting up AMD environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="🎉 AMD VPS Creation Successful", color=discord.Color.green())
            ...
            embed.add_field(name="ℹ️ Note", value="This is a AMD VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"✅ AMD VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your AMD VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All AMD VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ AMD VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="AMD VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your AMD VPS

Or use direct SSH with the provided command.
""", inline=False)
        
        await ctx.author.send(embed=embed)
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your AMD VPS.", ephemeral=True)
...
async def admin_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="AMD System Statistics", color=discord.Color.blue())
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your AMD VPS Usage", color=discord.Color.blue())
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="AMD Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling AMD bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "AMD-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"AMD VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"AMD VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ AMD VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"AMD VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ AMD VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"AMD VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ AMD VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling AMD VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    ports={'22/tcp': str(vps['external_ssh_port'])},
                    volumes={
                        f'AMD-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"AMD VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ AMD VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling AMD VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ AMD VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="AMD VPS Transferred to You", color=discord.Color.green())
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"AMD VPS Management - {vps_id}", color=discord.Color.blue())
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ AMD VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
...
# --- VPS plans (5 GB – 64 GB range) ---
PLANS = {
    "basic":   {"memory": 5,  "cpu": 1, "disk": 25,  "price": 25},
    "starter": {"memory": 8,  "cpu": 2, "disk": 40,  "price": 40},
    "advanced":{"memory": 16, "cpu": 4, "disk": 80,  "price": 70},
    "pro":     {"memory": 32, "cpu": 6, "disk": 160, "price": 130},
    "ultra":   {"memory": 64, "cpu": 8, "disk": 320, "price": 250},
}
...
def format_plans():
    ...
    for name, p in PLANS.items():
        lines.append(
            f"**{name.capitalize()}** — "
            f"{p['memory']} GB RAM / {p['cpu']} CPU / {p['disk']} GB Disk "
            f"→ **{p['price']} coins**"
        )
    return "\n".join(lines)
    ...
    def get_balance(self, user_id):
        self.cursor.execute('SELECT value FROM usage_stats WHERE key = ?', (f'coins:{user_id}',))
        row = self.cursor.fetchone()
        return int(row[0]) if row else 0

    def set_balance(self, user_id, amount):
        self.cursor.execute('INSERT OR REPLACE INTO usage_stats (key, value) VALUES (?, ?)', (f'coins:{user_id}', int(amount)))
        self.conn.commit()

    def add_coins(self, user_id, amount):
        current = self.get_balance(user_id)
        self.set_balance(user_id, current + int(amount))
    ...
    def get_last_daily(self, user_id):
        self.cursor.execute('SELECT value FROM usage_stats WHERE key = ?', (f'last_daily:{user_id}',))
        row = self.cursor.fetchone()
        return float(row[0]) if row else 0.0

    def set_last_daily(self, user_id, ts=None):
        ts = ts or time.time()
        self.cursor.execute('INSERT OR REPLACE INTO usage_stats (key, value) VALUES (?, ?)', (f'last_daily:{user_id}', int(ts)))
        self.conn.commit()



@bot.hybrid_command(name='daily', description='Claim daily coins')
@app_commands.describe(amount='Optional override amount (admin only)')
async def daily(ctx, amount: Optional[int] = None):
    ...
    if amount and has_admin_role(ctx):
        added = db.add_coins(user_id, amount)
        await ctx.send(f"✅ Gave {amount} coins to {ctx.author.mention}. New balance: {db.get_balance(user_id)} coins.", ephemeral=True)
        return

    if now - last < cooldown:
        remaining = int((cooldown - (now - last)) // 3600)
        await ctx.send(f"❌ You've already claimed daily. Try again in about {remaining} hour(s).", ephemeral=True)
        return

    reward = 1  # daily reward: 1 coin
    db.add_coins(user_id, reward)
    db.set_last_daily(user_id, now)
    await ctx.send(f"🎉 You claimed **{reward} coin**! Your new balance: **{db.get_balance(user_id)} coins**.", ephemeral=True)


@bot.hybrid_command(name='plans', description='Show available VPS plans')
async def plans(ctx):
    embed = discord.Embed(title="AMD VPS Plans", color=discord.Color.blurple())
    embed.description = format_plans()
    await ctx.send(embed=embed)


@bot.hybrid_command(name='buy', description='Buy a VPS plan using coins — creates VPS automatically')
@app_commands.describe(plan='Plan name to buy (e.g. basic, starter)')
async def buy(ctx, plan: str):
    plan = plan.lower()
    if plan not in PLANS:
        await ctx.send("❌ Plan not found. Use `/plans` to see available plans.", ephemeral=True)
        return
    ...
    if db.get_balance(user_id) < price:
        await ctx.send(f"❌ You need **{price} coins** to buy **{plan}** but you have **{db.get_balance(user_id)}**.", ephemeral=True)
        return

    if db.get_user_vps_count(int(user_id)) >= db.get_setting('max_vps_per_user', MAX_VPS_PER_USER):
        await ctx.send("❌ You already have the maximum number of VPS instances allowed.", ephemeral=True)
        return

    db.deduct_coins(user_id, price)
    ...
    try:
        status_msg = await ctx.send("🚀 Creating your VPS — please wait...")
        memory_bytes = memory * 1024 * 1024 * 1024
        vps_id = generate_vps_id()
        username = "root"
        password = "root"
        ...
        container = bot.docker_client.containers.run(
            ...
            network=DOCKER_NETWORK,
            ports={'22/tcp': str(external_port)},
            volumes={f'AMD-{vps_id}': {'bind': '/data', 'mode': 'rw'}},
            restart_policy={"Name": "always"}
        )

        await asyncio.sleep(3)
        setup_success, _ = await setup_container(container.id, status_msg, memory, vps_id=vps_id, use_custom_image=False)
        if not setup_success:
            db.add_coins(user_id, price)
            await status_msg.edit(content=f"❌ VPS setup failed. Refunded {price} coins.")
            return
        ...
        ssh_session_line = await capture_ssh_session_line(exec_cmd) or "N/A"
        ...
        embed = discord.Embed(title="🎉 VPS Purchased & Created", color=discord.Color.green())
        embed.add_field(name="VPS ID", value=vps_id, inline=True)
        embed.add_field(name="Plan", value=plan, inline=True)
        embed.add_field(name="SSH Port", value=str(external_port), inline=True)
        embed.add_field(name="Tmate", value=f"```{ssh_session_line}```", inline=False)
        await owner.send(embed=embed)
        await status_msg.edit(content=f"✅ VPS {vps_id} created and sent to your DMs! You were charged **{price} coins**.")
    except Exception as e:
        logger.error(f"Error in buy command: {e}")
        db.add_coins(user_id, price)
        await ctx.send(f"❌ Error while creating VPS: {e}", ephemeral=True)


@bot.hybrid_command(name='downtime', description='Give coins to all VPS owners (Admin only)')
@app_commands.describe(amount='Amount of coins to distribute to each VPS owner')
async def downtime(ctx, amount: int):
    if not has_admin_role(ctx):
        await ctx.send("❌ You must be an admin to run this.", ephemeral=True)
        return
    if amount <= 0:
    ...
    owners = {}
    for token, vps in all_vps.items():
        owner_id = vps.get('created_by')
        if owner_id:
            owners[owner_id] = owners.get(owner_id, 0) + 1

    for owner_id in owners.keys():
        try:
            db.add_coins(owner_id, amount)
            try:
                user = await bot.fetch_user(int(owner_id))
                await user.send(f"🎁 You received **{amount} coins** from admin downtime distribution. Your balance: {db.get_balance(owner_id)} coins.")
            except:
                pass
        except Exception as e:
            logger.error(f"Failed to give downtime coins to {owner_id}: {e}")

    await ctx.send(f"✅ Distributed **{amount} coins** to {len(owners)} VPS owners.", ephemeral=True)
