...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('Amdx7_bot.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('Amdx7Bot')
...
WATERMARK = "Amdx7 VPS Service"
WELCOME_MESSAGE = "Welcome To Amdx7! Get Started With Us!"
MAX_VPS_PER_USER = int(os.getenv('MAX_VPS_PER_USER', '3'))
DEFAULT_OS_IMAGE = os.getenv('DEFAULT_OS_IMAGE', 'ubuntu:22.04')
DOCKER_NETWORK = os.getenv('DOCKER_NETWORK', 'bridge')
MAX_CONTAINERS = int(os.getenv('MAX_CONTAINERS', '100'))
DB_FILE = 'Amdx7.db'
BACKUP_FILE = 'Amdx7_backup.pkl'
...
RUN echo '{welcome_message}' > /etc/motd && \\
    ...
    echo 'Amdx7-{vps_id}' > /etc/hostname
...
async def build_custom_image(vps_id, username, root_password, user_password, base_image=DEFAULT_OS_IMAGE):
    ...
    try:
        ...
        # Build the image
        image_tag = f"Amdx7/{vps_id.lower()}:latest"
...
async def setup_container(container_id, status_msg, memory, username, vps_id=None, use_custom_image=False):
    """Enhanced container setup with Amdx7 customization"""
    try:
        ...
        # Set Amdx7 customization
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("🎨 Setting up Amdx7 customization...", ephemeral=True)
        else:
            await status_msg.edit(content="🎨 Setting up Amdx7 customization...")
        ...
        hostname_cmd = f"echo 'Amdx7-{vps_id}' > /etc/hostname && hostname Amdx7-{vps_id}"
        ...
        if isinstance(status_msg, discord.Interaction):
            await status_msg.followup.send("✅ Amdx7 VPS setup completed successfully!", ephemeral=True)
        else:
            await status_msg.edit(content="✅ Amdx7 VPS setup completed successfully!")
...
bot = Amdx7Bot(command_prefix='/', intents=intents, help_command=None)
...
async def on_ready():
    ...
    try:
        await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name="Amdx7 VPS"))
...
async def show_commands(ctx):
    ...
    try:
        embed = discord.Embed(title="🤖 Amdx7 VPS Bot Commands", color=discord.Color.blue())
...
@app_commands.describe(
    ...
    use_custom_image="Use custom Amdx7 image (recommended)"
)
async def create_vps_command(ctx, memory: int, cpu: int, disk: int, owner: discord.Member, 
    ...
    try:
        ...
        status_msg = await ctx.send("🚀 Creating Amdx7 VPS instance... This may take a few minutes.")
        ...
        if use_custom_image:
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    volumes={
                        f'Amdx7-{vps_id}': {'bind': '/data', 'mode': 'rw'}
                    },
        ...
        await status_msg.edit(content="🔧 Container created. Setting up Amdx7 environment...")
        await asyncio.sleep(5)
        ...
        try:
            embed = discord.Embed(title="🎉 Amdx7 VPS Creation Successful", color=discord.Color.green())
            embed.add_field(name="🆔 VPS ID", value=vps_id, inline=True)
            embed.add_field(name="💾 Memory", value=f"64GB", inline=True)
            embed.add_field(name="⚡ CPU", value=f"10 cores", inline=True)
            embed.add_field(name="💿 Disk", value=f"190GB", inline=True)
            ...
            embed.add_field(name="ℹ️ Note", value="This is a Amdx7 VPS instance. You can install and configure additional packages as needed.", inline=False)
            
            await owner.send(embed=embed)
            await status_msg.edit(content=f"✅ Amdx7 VPS creation successful! VPS has been created for {owner.mention}. Check your DMs for connection details.")
...
async def list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your Amdx7 VPS Instances", color=discord.Color.blue())
...
async def admin_list_vps(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="All Amdx7 VPS Instances", color=discord.Color.blue())
        valid_vps_count = 0
        ...
        for token, vps in all_vps.items():
            ...
            status = vps.get('status', "online").capitalize()
...
async def delete_vps(ctx, vps_id: str):
    ...
    try:
        ...
        await ctx.send(f"✅ Amdx7 VPS {vps_id} has been deleted successfully!")
    except Exception as e:
...
async def connect_vps(ctx, token: str):
    ...
    try:
        ...
        embed = discord.Embed(title="Amdx7 VPS Connection Details", color=discord.Color.blue())
...
4. You will be connected to your Amdx7 VPS
...
""".format(username=vps["username"]), inline=False)
        ...
        await ctx.send("✅ Connection details sent to your DMs! Use the Tmate command to connect to your Amdx7 VPS.", ephemeral=True)
...
async def vps_usage(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="Your Amdx7 VPS Usage", color=discord.Color.blue())
        embed.add_field(name="Total VPS Instances", value=len(user_vps), inline=True)
        embed.add_field(name="Total Memory Allocated", value=f"64GB", inline=True)
        embed.add_field(name="Total CPU Cores Allocated", value=10, inline=True)
        embed.add_field(name="Total Disk Allocated", value=f"190GB", inline=True)
...
async def global_stats(ctx):
    ...
    try:
        ...
        embed = discord.Emembed(title="Amdx7 Global Usage Statistics", color=discord.Color.blue())
...
async def reinstall_bot(ctx):
    ...
    try:
        await ctx.send("🔄 Reinstalling Amdx7 bot... This may take a few minutes.")
...
"""
        ...
        process = await asyncio.create_subprocess_exec(
            "docker", "build", "-t", "Amdx7-bot", "-f", "Dockerfile.bot", ".",
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE
        )
...
class VPSManagementView(ui.View):
    ...
    async def handle_missing_container(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(title=f"Amdx7 VPS Management - {self.vps_id}", color=discord.Color.red())
    ...
    async def start_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Embed(title=f"Amdx7 VPS Management - {self.vps_id}", color=discord.Color.green())
            ...
            await interaction.followup.send("✅ Amdx7 VPS started successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def stop_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            embed = discord.Emembed(title=f"Amdx7 VPS Management - {self.vps_id}", color=discord.Color.orange())
            ...
            await interaction.message.edit(embed=embed)
            await interaction.followup.send("✅ Amdx7 VPS stopped successfully!", ephemeral=True)
        except Exception as e:
    ...
    async def restart_vps(self, interaction: discord.Interaction, button: discord.ui.Button):
        try:
            ...
            if token:
                ...
                try:
                    ...
                    if ssh_session_line:
                        ...
                        try:
                            owner = await bot.fetch_user(int(vps["created_by"]))
                            embed = discord.Embed(title=f"Amdx7 VPS Restarted - {self.vps_id}", color=discord.Color.blue())
            ...
            await interaction.followup.send("✅ Amdx7 VPS restarted successfully! New SSH details sent to owner.", ephemeral=True)
        except Exception as e:
...
class OSSelectionView(ui.View):
    ...
    async def reinstall_os(self, interaction: discord.Interaction, image: str):
        try:
            ...
            status_msg = await interaction.followup.send("🔄 Reinstalling Amdx7 VPS... This may take a few minutes.", ephemeral=True)
            ...
            try:
                container = bot.docker_client.containers.run(
                    ...
                    network=DOCKER_NETWORK,
                    volumes={
                        f'Amdx7-{self.vps_id}': {'bind': '/data', 'mode': 'rw'}
                    }
                )
            except docker.errors.ImageNotFound:
            ...
            try:
                ...
                if ssh_session_line:
                    ...
                    try:
                        owner = await bot.fetch_user(int(vps["created_by"]))
                        embed = discord.Embed(title=f"Amdx7 VPS Reinstalled - {self.vps_id}", color=discord.Color.blue())
            ...
            await status_msg.edit(content="✅ Amdx7 VPS reinstalled successfully!")
            ...
            try:
                ...
                try:
                    ...
                    await channel.send(f"❌ Error reinstalling Amdx7 VPS {self.vps_id}: {str(e)}")
                except:
...
class TransferVPSModal(ui.Modal, title='Transfer VPS'):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message(f"✅ Amdx7 VPS {self.vps_id} has been transferred from {old_owner_name} to {new_owner_name}!", ephemeral=True)
            
            try:
                embed = discord.Embed(title="Amdx7 VPS Transferred to You", color=discord.Color.green())
...
async def manage_vps(ctx, vps_id: str):
    ...
    try:
        ...
        embed = discord.Embed(title=f"Amdx7 VPS Management - {vps_id}", color=discord.Color.blue())
        embed.add_field(name="Status", value=f"{status} (Container: {container_status})", inline=True)
        embed.add_field(name="Memory", value=f"64GB", inline=True)
        embed.add_field(name="CPU", value=f"10cores", inline=True)
        embed.add_field(name="Disk Allocated", value=f"190GB", inline=True)
        embed.add_field(name="Username", value=vps['username'], inline=True)
        embed.add_field(name="Created", value=vps['created_at'], inline=True)
...
async def transfer_vps_command(ctx, vps_id: str, new_owner: discord.Member):
    ...
    try:
        ...
        await ctx.send(f"✅ Amdx7 VPS {vps_id} has been transferred from {ctx.author.name} to {new_owner.name}!")
